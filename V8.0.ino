...
void dashboardPage(EthernetClient &client) 
{
  ...
  for ( i = 0; i < maxrelay ; i++ )
  {
    ...
    client.println("<fONt color=#ffffff> "+ RelayState[i] + "</td><td align=center>");
      ...
      if(RelayState[i] == "OFF")
    {
        client.println("<a href=\"/R"+ String(RelayPin[i]) +"_ON\"><button>ON</button></a></td></tr>");
      }      
      else if(RelayState[i] == "ON")
    {
        client.println("<a href=\"/R"+ String(RelayPin[i]) +"_OFF\"><button>OFF</button></a></td></tr>");                                                                    
      }    
  }
  ...
  for ( i = 0; i < maxrelay ; i++ )
  {
    client.println("<tr><td>"+ RelayName[i] +"</td><td><form><input type=number length=2  name=V"+ String(RelayPin[i]) +"_ON value="+ String(RelayTimeOn[i]) +"><td><input type=submit value=Submit></form></td>");
    client.println("<td><form><input type=number length=2  name=V"+ String(RelayPin[i]) +"_OFF value="+ String(RelayTimeOff[i]) +"><td><input type=submit value=Submit></form></td></tr>");
  }
    ...
    while (client.connected()) 
    {
      if (client.available()) 
      {
        ...
        if (c == '\n') 
        {
          for ( i = 0; i < maxrelay ; i++ )
          {
            if  (strstr(linebuf, (String("GET /R") + String(RelayPin[i]) + String("_OFF")).c_str()) != NULL)
            {
              digitalWrite(RelayPin[i], HIGH);
              RelayState[i] = "OFF";
            }
            else if  (strstr(linebuf, (String("GET /R") + String(RelayPin[i]) + String("_ON")).c_str()) != NULL)
            {
            digitalWrite(RelayPin[i], LOW);
            RelayState[i] = "ON";
            }
          ...
          for ( i = 0; i < maxrelay ; i++ )
          {
              if  (strstr(linebuf, (String("V") + String(RelayPin[i]) + String("_ON")).c_str()) != NULL)
            {
              Cadenas = linebuf;
          ...
          for ( i = 0; i < maxrelay ; i++ )
          {
            if  (strstr(linebuf, (String("V") + String(RelayPin[i]) + String("_OFF")).c_str()) != NULL)
            {
              Cadenas = linebuf;
              int j = Cadenas.indexOf("=");
              if (j>30)
